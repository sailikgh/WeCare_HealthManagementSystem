    ...
    <?php
        ...
        if(mysqli_num_rows($result) > 0)
        {
            ...
            { 
                ...
                echo "<tr><td>". $sec ."</td><td>". $phone ."</td></tr>";
            }
            echo("</table>");
            ...
            <a href="deptform.php?id=<?php echo ($hosid)?>" class="w3-button w3-teal"><b>+</b> Add New</a>
        <?php
        echo("<table class='w3-table-all w3-card-4' id='myTable'>
        <tr class='w3-teal'>
          <th>ID</th>
        ...
        $stmt = $con->prepare("select * from wc_hos_dept a join wc_dept b where a.dept_id=b.dept_id and hos_id=?;");
        $stmt->bind_param('i', $hosid);
        $stmt->execute();
        $result = $stmt->get_result();
        if(mysqli_num_rows($result) > 0)
        {
            ...
            { 
                ...
                <td>". $row['dept_phone'] ."</td><td>". $row['dept_floor'] ."</td>
                <td>". $row['dept_bldg'] ."</td>
                <td>". "<a href=deptdel.php?id=".$row['dept_id'].">Delete</a>" ."</td></tr>";
            }
            echo("</table>");
        }
    }
