...
<div class="w3-row-padding w3-center w3-padding-64" id="pricing">
    ...
    <?php
        ...
        $stmt = $con->prepare("select * from wc_dept a join wc_hos_dept b where a.dept_id=b.dept_id and b.hos_id=?;");
        $stmt->bind_param('i', $hosid);
        $stmt->execute();
        $result = $stmt->get_result();
        if(mysqli_num_rows($result) > 0)
        {
            while ($row = $result->fetch_assoc())
            { 
              echo "<tr><td>". $row['dept_id'] ."</td><td>". $row['dept_name'] ."</td><td>". $row['dept_phone'] ."</td><td>". $row['dept_bldg'] ."</td><td>". $row['dept_floor'] ."</td></tr>";
            }
